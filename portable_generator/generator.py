...
def clear_generation_output(output_dir: Path) -> tuple[int, int]:
    """Remove old generated images and their YOLO annotations."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum number of random units per image (second range, inclusive)",
    )
    parser.add_argument(
        "--units-max-second",
        type=int,
        default=15,
        help="Maximum number of random units per image (second range, inclusive)",
    )
    ...
    if args.units_min_second < 0:
        parser.error("--units-min-second must be non-negative")
    if args.units_max_second < args.units_min_second:
        parser.error("--units-max-second must be greater than or equal to --units-min-second")

    removed_images, removed_labels = clear_generation_output(args.output)
    print(
        f"Cleared output directory: {removed_images} images and "
        f"{removed_labels} annotation files removed"
    )
    generator = Generator(
        seed=args.seed,
    ...
    for index in range(args.count):
        ...
        generator.add_tower()
        generator.add_unit(n=random.choice([random.randint(args.units_min, args.units_max), random.randint(args.units_min_second, args.units_max_second)]))
        generator.build(
