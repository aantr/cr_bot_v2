...
WINDOW_NAME = "Build bars dataset"
BAR_CLASS_IDS = (0, 1)
...
def filter_bar_detections(result):
    """Keep only 0=bar and 1=bar-level in both the preview and saved labels."""
    if result.boxes is None or len(result.boxes) == 0:
        return result
    class_ids = result.boxes.cls
    keep = (class_ids == BAR_CLASS_IDS[0]) | (class_ids == BAR_CLASS_IDS[1])
